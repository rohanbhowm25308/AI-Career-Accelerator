...
# =============================================================================
# AI Interview Replay - a timestamped walkthrough of the answers actually
# given, honestly estimated: this app doesn't record audio/video, so
# timestamps are calculated from answer length at an average speaking pace
# (~130 words/min), not a real recording. Clearly disclosed in the output.
# =============================================================================

_WORDS_PER_SECOND = 130 / 60


def build_replay(session):
    answers = session.get("answers", [])
    if not answers:
        return {"markers": [], "total_estimated_seconds": 0, "note": "No answers recorded yet."}

    markers = []
    cursor = 0.0
    for i, record in enumerate(answers, start=1):
        word_count = len(record["answer"].split())
        duration = max(3.0, word_count / _WORDS_PER_SECOND)
        score = record["feedback"]["score"]

        if word_count < 8:
            tag, note = "brief", "Very short answer - likely a pause or hesitation here."
        elif score >= 80:
            tag, note = "strong", "Strong, confident answer."
        elif score >= 55:
            tag, note = "solid", "Solid answer - could be sharper with more specifics."
        else:
            tag, note = "weak", "Needs more specifics or structure."

        markers.append({
            "index": i,
            "question": record["question"]["text"] if isinstance(record["question"], dict) else str(record["question"]),
            "start_seconds": round(cursor),
            "end_seconds": round(cursor + duration),
            "word_count": word_count,
            "tag": tag,
            "note": note,
            "score": score,
        })
        cursor += duration + 2  # ~2s assumed gap between question and next answer

    return {
        "markers": markers,
        "total_estimated_seconds": round(cursor),
        "note": "Timestamps are estimated from each answer's length at an average speaking pace - this app doesn't record audio/video, so these are calculated markers, not a real recording.",
    }
